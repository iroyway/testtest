...
guaopencard_addSku = $.isNode()
  ? process.env.guaopencard_addSku104
    ? process.env.guaopencard_addSku104
    : `${guaopencard_addSku}`
  : $.getdata("guaopencard_addSku104")
  ? $.getdata("guaopencard_addSku104")
  : `${guaopencard_addSku}`;
guaopencard_addSku = $.isNode()
  ...
  : `${guaopencard_addSku}`;
guaopencard = $.isNode()
  ? process.env.guaopencard104
    ? process.env.guaopencard104
    : `${guaopencard}`
  : $.getdata("guaopencard104")
  ? $.getdata("guaopencard104")
  : `${guaopencard}`;
guaopencard = $.isNode()
  ...
  : `${guaopencard}`;
guaopenwait = $.isNode()
  ? process.env.guaopenwait104
    ? process.env.guaopenwait104
    : `${guaopenwait}`
  : $.getdata("guaopenwait104")
  ? $.getdata("guaopenwait104")
  : `${guaopenwait}`;
guaopenwait = $.isNode()
...
guaopencard_draw = $.isNode()
  ? process.env.guaopencard_draw104
    ? process.env.guaopencard_draw104
    : guaopencard_draw
  : $.getdata("guaopencard_draw104")
  ? $.getdata("guaopencard_draw104")
  : guaopencard_draw;
guaopencard_draw = $.isNode()
...
!(async () => {
  ...
  $.MixNicks = "";
  $.inviteNick =
    "RXIs1ynq8Qy8DQfl9OyL614tLNYA4seuA67MOIYQxEk3Vl9+AVo4NF+tgyeIc6A6kdK3rLBQpEQH9V4tdrrh0w==";
...
async function run() {
  try {
    ...
    if (guaopencard_addSku + "" == "true") {
      ...
      let goodsArr = [];
      if (cleanCart) {
        goodsArr = await cleanCart.clean(
          cookie,
          "https://jd.smiek.tk/jdcleancatr_21102717",
          ""
        );
        await takePostRequest("mission");
        await $.wait(parseInt(Math.random() * 2000 + 3000, 10));
    ...
    if (guaopencard_draw + "" !== "0") {
      ...
      console.log('如需抽奖请设置环境变量[guaopencard_draw104]为"3" 3为次数');
...
async function takePostRequest(type) {
  ...
  switch (type) {
    case "isvObfuscator":
      ...
      break;
    case "activity_load":
      url = `${domain}/dm/front/openCardNew/activity_load?mix_nick=${
        $.MixNick || $.MixNicks || ""
      }`;
      admJson = {
        jdToken: $.Token,
      ...
      break;
    case "shopList":
      url = `${domain}/dm/front/openCardNew/shopList?mix_nick=${
        $.MixNick || $.MixNicks || ""
      }`;
      admJson = {};
      ...
      break;
    case "绑定":
      url = `${domain}/dm/front/openCardNew/complete/mission?mix_nick=${
        $.MixNick || $.MixNicks || ""
      }`;
      admJson = {
    ...
    case "mission":
      ...
      break;
    case "抽奖":
      url = `${domain}/dm/front/openCardNew/draw/post?mix_nick=${
        $.MixNick || $.MixNicks || ""
      }`;
      ...
      break;
    case "followShop":
      url = `${domain}/dm/front/openCardNew/followShop?mix_nick=${
        $.MixNick || $.MixNicks || ""
      }`;
      ...
      break;
    case "addCart":
      url = `${domain}/dm/front/openCardNew/addCart?mix_nick=${
        $.MixNick || $.MixNicks || ""
      }`;
      ...
      break;
    case "myAward":
      url = `${domain}/dm/front/openCardNew/myAwards?mix_nick=${
        $.MixNick || $.MixNicks || ""
      }`;
    ...
    case "missionInviteList":
      url = `${domain}/dm/front/openCardNew/missionInviteList?mix_nick=${
        $.MixNick || $.MixNicks || ""
      }`;
      admJson = {
        inviteListRequest: {
        ...
        if (typeof res == "object") {
          if (res.success && res.success === true && res.data) {
            if (res.data.status && res.data.status == 200) {
              ...
              if (
                type != "setMixNick" &&
                (res.msg || res.data.isOpenCard || res.data.remark)
              )
                console.log(
                  `${(title && title + ":") || ""}${
                    res.msg || res.data.isOpenCard || res.data.remark || ""
              ...
              if (type == "activity_load") {
                if (res.msg || res.data.isOpenCard) {
                  if (
                    (res.msg || res.data.isOpenCard || "").indexOf("绑定成功") >
                    -1
                  )
                    $.toBind = 1;
                }
                if (res.data) {
                  ...
                  $.hasCollectShop =
                    res.data.missionCustomer.hasCollectShop || 0;
                }
